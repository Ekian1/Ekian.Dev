...
const PF = {
    ...
    selectionCanvas: null, sctx: null,
    ...
    selection: null, // {type, x, y, w, h} or path or raster
    clipboard: null,
...
async function init() {
    ...
    PF.selectionCanvas = document.createElement('canvas');
    PF.sctx = PF.selectionCanvas.getContext('2d');
...
function setCanvasSize(w, h) {
    ...
    if (!PF.selectionCanvas) {
        PF.selectionCanvas = document.createElement('canvas');
        PF.sctx = PF.selectionCanvas.getContext('2d');
    }
    const scratch = document.createElement('canvas');
    scratch.width = w; scratch.height = h;
    scratch.getContext('2d').drawImage(PF.selectionCanvas, 0, 0);
    PF.selectionCanvas.width = w; PF.selectionCanvas.height = h;
    PF.sctx.drawImage(scratch, 0, 0);
...
function renderAll() {
    ...
    for (let i = PF.layers.length - 1; i >= 0; i--) {
        ...
        PF.ctx.drawImage(l.canvas, l.x, l.y);
    }
...
function saveHistory(label) {
    ...
    const snapshot = PF.layers.map(l => {
        ...
        return { name: l.name, x: l.x, y: l.y, canvas: c, visible: l.visible, locked: l.locked, opacity: l.opacity, blendMode: l.blendMode };
    });
    
    // Also save selection mask
    const selCanvas = document.createElement('canvas');
    selCanvas.width = PF.width; selCanvas.height = PF.height;
    selCanvas.getContext('2d').drawImage(PF.selectionCanvas, 0, 0);

    PF.history.push({ label, layers: snapshot, activeIndex: PF.activeLayerIndex, w: PF.width, h: PF.height, selection: PF.selection, selectionMask: selCanvas });
...
function restoreHistory() {
    ...
    PF.selection = state.selection ? JSON.parse(JSON.stringify(state.selection)) : null;
    PF.sctx.clearRect(0, 0, PF.width, PF.height);
    if (state.selectionMask) PF.sctx.drawImage(state.selectionMask, 0, 0);
...
function drawSelection() {
    ...
    if (s.type === 'raster') {
        // Draw the raster mask faintly for Wand selection
        PF.octx.globalAlpha = 0.4;
        PF.octx.fillStyle = '#4a90ff';
        PF.octx.globalCompositeOperation = 'source-over';
        // We tint the overlay where the mask is white
        PF.octx.fillRect(0, 0, PF.width, PF.height);
        PF.octx.globalCompositeOperation = 'destination-in';
        PF.octx.drawImage(PF.selectionCanvas, 0, 0);
