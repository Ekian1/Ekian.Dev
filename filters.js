...
function showFilterModal(title, applyFn) {
    ...
    switch (title) {
        ...
        case 'Posterize':
            body = `<label>Levels</label><input type="range" id="f-posterize" min="2" max="25" value="4"><span id="f-posterize-v" style="color:#888">4</span>`; break;
        case 'Pixelate':
            body = `<label>Block Size</label><input type="range" id="f-pixelate" min="2" max="100" value="10"><span id="f-pixelate-v" style="color:#888">10</span>`; break;
    }
...
function showReplaceBgModal() {
    const body = `
        <label>Screen Color (to remove)</label>
        <input type="color" id="f-rbg-key" value="#00ff00">
        <label>Tolerance</label>
        <input type="range" id="f-rbg-tol" min="1" max="200" value="80"><span id="f-rbg-tol-v" style="color:#888">80</span>
        <label style="margin-top:12px">Replace With</label>
        <div style="display:flex;gap:8px;margin-top:6px">
            <button class="btn-secondary" id="f-rbg-solid-btn" style="flex:1;padding:6px;border-radius:6px;cursor:pointer;border:2px solid var(--accent);">Solid Color</button>
            <button class="btn-secondary" id="f-rbg-image-btn" style="flex:1;padding:6px;border-radius:6px;cursor:pointer;border:1px solid var(--border);">Upload Image</button>
        </div>
        <div id="f-rbg-solid-opts" style="margin-top:8px">
            <input type="color" id="f-rbg-color" value="#ffffff" style="width:100%;height:36px">
        </div>
        <div id="f-rbg-image-opts" style="margin-top:8px;display:none">
            <input type="file" id="f-rbg-file" accept="image/*" style="width:100%;background:var(--bg-input);border:1px solid var(--border);color:var(--text);padding:6px;border-radius:6px;font-size:12px">
            <p id="f-rbg-file-name" style="color:var(--text-muted);font-size:11px;margin-top:4px"></p>
        </div>
    `;
    showModal('Replace Background', body, [
        { text: 'Cancel' },
        { text: 'Apply', primary: true, action: () => filterReplaceBg() }
    ]);
    // Wire tabs
    setTimeout(() => {
        const solidBtn = document.getElementById('f-rbg-solid-btn');
        const imageBtn = document.getElementById('f-rbg-image-btn');
        const solidOpts = document.getElementById('f-rbg-solid-opts');
        const imageOpts = document.getElementById('f-rbg-image-opts');
        const tolSlider = document.getElementById('f-rbg-tol');
        const tolVal = document.getElementById('f-rbg-tol-v');

        tolSlider.addEventListener('input', () => tolVal.textContent = tolSlider.value);

        solidBtn.addEventListener('click', () => {
            solidBtn.style.borderColor = 'var(--accent)'; solidBtn.style.borderWidth = '2px';
            imageBtn.style.borderColor = 'var(--border)'; imageBtn.style.borderWidth = '1px';
            solidOpts.style.display = ''; imageOpts.style.display = 'none';
            window._rbgMode = 'solid';
        });
        imageBtn.addEventListener('click', () => {
            imageBtn.style.borderColor = 'var(--accent)'; imageBtn.style.borderWidth = '2px';
            solidBtn.style.borderColor = 'var(--border)'; solidBtn.style.borderWidth = '1px';
            solidOpts.style.display = 'none'; imageOpts.style.display = '';
            window._rbgMode = 'image';
        });

        document.getElementById('f-rbg-file').addEventListener('change', (e) => {
            const file = e.target.files[0];
            if (file) {
                document.getElementById('f-rbg-file-name').textContent = file.name;
                const reader = new FileReader();
                reader.onload = (ev) => { window._rbgImageData = ev.target.result; };
                reader.readAsDataURL(file);
            }
        });

        window._rbgMode = 'solid';
        window._rbgImageData = null;
    }, 50);
}

function filterReplaceBg() {
    const keyHex = document.getElementById('f-rbg-key').value;
    const tol = +document.getElementById('f-rbg-tol').value;
    const keyColor = hexToRgb(keyHex);
    ...
    // Step 1: Build a mask of the green screen area (white = green screen)
    ...
    for (let y = 0; y < h; y++) {
        for (let x = 0; x < w; x++) {
            ...
            if (dist < tol) {
                // Fully green — mark mask as white, remove from layer
                md[i] = 255; md[i+1] = 255; md[i+2] = 255; md[i+3] = 255;
                d[i+3] = 0;
                if (x < minX) minX = x; if (x > maxX) maxX = x;
                if (y < minY) minY = y; if (y > maxY) maxY = y;
            } else if (dist < tol * 1.4) {
                const fade = (dist - tol) / (tol * 0.4);
                const maskAlpha = clamp(255 * (1 - fade));
                md[i] = 255; md[i+1] = 255; md[i+2] = 255; md[i+3] = maskAlpha;
                d[i+3] = clamp(a * fade);
                if (x < minX) minX = x; if (x > maxX) maxX = x;
                if (y < minY) minY = y; if (y > maxY) maxY = y;
            }
    ...
    const gsX = minX, gsY = minY;
    const gsW = maxX - minX + 1, gsH = maxY - minY + 1;
    const hasRegion = maxX >= minX && maxY >= minY;

    // Helper: composite the replacement onto the layer
    function compositeReplacement(replaceCanvas) {
        // Clip replacement to the green screen mask shape
        const clipped = document.createElement('canvas');
        clipped.width = w; clipped.height = h;
        const cctx = clipped.getContext('2d');
        cctx.drawImage(replaceCanvas, 0, 0);
        cctx.globalCompositeOperation = 'destination-in';
        cctx.drawImage(maskCanvas, 0, 0);

        // Draw clipped replacement BEHIND the foreground on the same layer
        // First put the keyed foreground on imgData
        layer.ctx.putImageData(imgData, 0, 0);
        // Then draw replacement behind using destination-over
        layer.ctx.globalCompositeOperation = 'destination-over';
        layer.ctx.drawImage(clipped, 0, 0);
        layer.ctx.globalCompositeOperation = 'source-over';

        renderAll(); saveHistory('Replace Background');
    }

    const mode = window._rbgMode || 'solid';
    if (mode === 'image' && window._rbgImageData && hasRegion) {
        const img = new Image();
        img.onload = () => {
            const rc = document.createElement('canvas');
            rc.width = w; rc.height = h;
            const rctx = rc.getContext('2d');
            // Cover-fit image into the green screen bounding box
            const scale = Math.max(gsW / img.width, gsH / img.height);
            const dw = img.width * scale, dh = img.height * scale;
            const dx = gsX + (gsW - dw) / 2, dy = gsY + (gsH - dh) / 2;
            rctx.drawImage(img, dx, dy, dw, dh);
            compositeReplacement(rc);
        };
        img.src = window._rbgImageData;
    } else {
        const rc = document.createElement('canvas');
        rc.width = w; rc.height = h;
        const rctx = rc.getContext('2d');
        rctx.fillStyle = (mode === 'solid') ? document.getElementById('f-rbg-color').value : '#ffffff';
        rctx.fillRect(0, 0, w, h);
        compositeReplacement(rc);
    }

    window._rbgMode = null;
    window._rbgImageData = null;
}
...
function filterPosterize() {
    const levels = +document.getElementById('f-posterize').value;
    ...
    saveHistory('Posterize');
}

function filterPixelate() {
    const pSize = +document.getElementById('f-pixelate').value;
    const layer = getActiveLayer();
    const tmp = document.createElement('canvas');
    tmp.width = Math.ceil(layer.canvas.width / pSize);
    tmp.height = Math.ceil(layer.canvas.height / pSize);
    const tctx = tmp.getContext('2d');
    tctx.imageSmoothingEnabled = false;
    tctx.drawImage(layer.canvas, 0, 0, tmp.width, tmp.height);
    
    layer.clear();
    layer.ctx.imageSmoothingEnabled = false;
    layer.ctx.drawImage(tmp, 0, 0, layer.canvas.width, layer.canvas.height);
    layer.ctx.imageSmoothingEnabled = true;

    renderAll();
    saveHistory('Pixelate');
}
...
function filterEdgeDetect() {
    ...
    const result = convolve(imgData, layer.canvas.width, layer.canvas.height, kernel);
